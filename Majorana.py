...
# -*- coding: utf-8 -*-
"""
Unified PAQJP+PJP — Exhaustive 65,535-Pair Tournament (verbose progress)
========================================================================
Prints one line per transform and one line per pair as they complete.
"""

import math, random, decimal, hashlib, base64, heapq, struct, os
import tempfile, re, sys, subprocess, importlib, time, urllib.request
import site
...
def _try_install_zstd(verbose=True):
    candidates = [
        [sys.executable, '-m', 'pip', 'install', '--no-input',
         '--disable-pip-version-check', 'zstandard'],
        [sys.executable, '-m', 'pip', 'install', '--user', '--no-input',
         '--disable-pip-version-check', 'zstandard'],
        [sys.executable, '-m', 'pip', 'install', '--break-system-packages',
         '--no-input', '--disable-pip-version-check', 'zstandard'],
    ...
    for cmd in candidates:
        ...
        try:
            ...
            if r.returncode == 0 and _try_import_zstd() is not None:
                if verbose:
                    print("  SUCCESS!")
                return True
            ...
            if verbose: print("    (command not found)")
...
print("=" * 70)
print("Checking optional backend: zstandard")
print("=" * 70)
_zstd = _try_import_zstd()
if _zstd is None:
    print("zstandard NOT installed. Attempting automatic installation...")
    if _try_install_zstd(verbose=True):
        _zstd = _try_import_zstd()
...
if _zstd is not None:
    ...
    zstd_dctx = None
    HAS_ZSTD = False
    print("zstandard UNAVAILABLE — continuing without it.")
    print("Backends in use: lzma=%s brotli=%s paq=%s raw=always"
          % ('Y' if HAS_LZMA else 'N',
             'Y' if HAS_BROTLI else 'N',
             'Y' if paq else 'N'))
...
if input("Option 2: Install paq + brotli? (y/n) [y]: ").strip().lower() != 'n':
    ...
    print("Skipping paq + brotli.")

print(f"\nBackends: zstd={'Y' if HAS_ZSTD else 'N'}  "
      f"lzma={'Y' if HAS_LZMA else 'N'}  "
      f"paq={'Y' if paq else 'N'}  "
      f"brotli={'Y' if HAS_BROTLI else 'N'}")

PROGNAME = "UnifiedPAQJP+PJP (Exhaustive 65,535-Pair Tournament)"
...
class UnifiedCompressor:
    ...
    def __init__(self, try_download=True):
        ...
        self.static_dict = build_real_dictionary(try_download=try_download)
        self.word_to_index = {w: i for i, w in enumerate(self.static_dict)}
    ...
    def transform_19(self, data):
        ...
        s = str(decimal.Decimal(1) / e).replace('.', '')[:max(10, len(data) // 2 + 5)]
        mask = bytes(int(s[i:i + 2]) % 256 for i in range(0, len(s), 2))
        t = bytearray(data)
        for i in range(len(t)): t[i] ^= mask[i % len(mask)]
        return bytes(t)
    ...
    def transform_20(self, data):
        ...
        decimal.getcontext().prec = 60
        e = decimal.Decimal(1).exp()
        s = str(decimal.Decimal(5) * e).replace('.', '')[:max(10, len(data) // 2 + 5)]
        mask = bytes(int(s[i:i + 2]) % 256 for i in range(0, len(s), 2))
        t = bytearray(data)
        for i in range(len(t)): t[i] ^= mask[i % len(mask)]
        return bytes(t)
    ...
    def transform_23(self, data):
        ...
        tb = bytes(out)
        try:
            if self.reverse_transform_23(tb) == data: return tb
        except Exception: pass
        return b'\x00' + data
    ...
    def _decompress_backend_with_flag(self, data):
        ...
        if f == 0: return p
        if f == 1:
            if not HAS_ZSTD: raise TransformError("zstd backend unavailable")
            return zstd_dctx.decompress(p)
    ...
    def transform_58(self, data):
        if not data:
            return (b'\x58' + b'\x00' + b'\x00' + (0).to_bytes(8, 'big')
                    + b'\x00' + b'\x00' * 256)
        ...
        if bd == 4:
            words = []
            for b in data:
                words.append((b >> 4) & 0xF); words.append(b & 0xF)
            mask = 0xF; wbytes = 1
        else:
            wbytes = bd // 8
            ...
            mask = (1 << bd) - 1
        n = len(words)
        fibs = self.fibonacci or [0, 1]; Lf = len(fibs)
        const = Counter(words).most_common(1)[0][0]
        lz_pred = [0] * n; last_after = {}
        for i in range(n):
            if i == 0: lz_pred[i] = const
            else:
    ...
    def reverse_transform_58(self, data):
        ...
        if bd == 4:
            ...
            const = int.from_bytes(data[pos:pos + wbytes], 'big'); pos += wbytes
            mask = (1 << bd) - 1
        if pos + 256 > len(data): raise TransformError("T58 short cl")
        ...
        for b in huff:
            for i in range(7, -1, -1): bits.append((b >> i) & 1)
        if bd == 4:
            n = orig_len * 2; n_rbytes = (n + 1) // 2
        else:
            n = (orig_len + wbytes - 1) // wbytes; n_rbytes = n * wbytes
        rbytes = bytearray(); bpos = 0; nb = len(bits)
        while len(rbytes) < n_rbytes and bpos < nb:
            ...
            for L in range(1, maxL + 1):
                ...
                if (L, v) in code_to:
                    rbytes.append(code_to[(L, v)]); bpos += L; found = True; break
            if not found: raise TransformError("T58 huff dec")
        if len(rbytes) != n_rbytes: raise TransformError("T58 huff len")
        if bd == 4:
            residuals = []
            for b in rbytes:
                residuals.append((b >> 4) & 0xF); residuals.append(b & 0xF)
            residuals = residuals[:n]
        elif bd == 16:
            residuals = [int.from_bytes(rbytes[i:i + 2], 'big') for i in range(0, len(rbytes), 2)]
        elif bd == 32:
            residuals = [int.from_bytes(rbytes[i:i + 4], 'big') for i in range(0, len(rbytes), 4)]
        else:
            residuals = [int.from_bytes(rbytes[i:i + 8], 'big') for i in range(0, len(rbytes), 8)]
        fibs = self.fibonacci or [0, 1]; Lf = len(fibs)
        words = [0] * n; last_after = {}
        for i in range(n):
            if pred_code == 0: p = const
            elif pred_code == 1: p = const if i == 0 else (words[i - 1] + fibs[i % Lf]) & mask
            else: p = const if i == 0 else last_after.get(words[i - 1], const)
            w = (residuals[i] + p) & mask
            words[i] = w
            if i >= 1: last_after[words[i - 1]] = w
        if bd == 4:
            out = bytearray()
        ...
        out = bytearray()
        for w in words: out.extend(w.to_bytes(wbytes, 'big'))
        return bytes(out[:orig_len])
    ...
    def _build_transform_maps(self):
        self.fwd_transforms = {}; self.rev_transforms = {}
        for i in range(1, 22):
    ...
    # ================================================================
    # ★  EXHAUSTIVE TOURNAMENT with verbose progress
    # ================================================================
    def compress_file_tournament(self, infile, include_pairs=True,
                                 include_multi=False, time_limit=None):
        try:
        ...
        print("=" * 64)
        print("Evaluating candidates (progress printed as each completes):")
        print("   raw payload")
        print("   .a1 ... .a256     (single transform)")
        if include_pairs:
            print("   .p1 ... .p65535   (all 65,535 pairs)")
        if include_multi:
            print("   multi-pair chains (2..3 pairs, random)")
        ...
        # ---- raw ----
        raw_c = self._compress_backend(data)
        candidates.append(("_raw_", raw_c, "raw payload"))
        best_size = len(raw_c)
        print(f"  [raw]                    payload={len(raw_c):>10} bytes  (best: {best_size})")

        # ---- 256 single transforms ----
        n_single = 0
        best_single_size = None
        for t in range(1, 257):
            if time_limit and time.time() - st > time_limit:
                print(f"  [single {t:>3}/256] TIME LIMIT reached, stopping singles.")
                break
            try:
                tr = self.fwd_transforms[t](data)
            except Exception as e:
                if self.VERBOSE_SINGLE:
                    print(f"  [single {t:>3}/256] #{t:<3} SKIPPED (forward error: {e})")
                continue
            if not self._verify_lossless(data, tr, self.rev_transforms[t]):
                if self.VERBOSE_SINGLE:
                    print(f"  [single {t:>3}/256] #{t:<3} REJECTED (not lossless)")
                continue
            try:
                payload = self._compress_backend(tr)
            except Exception as e:
                if self.VERBOSE_SINGLE:
                    print(f"  [single {t:>3}/256] #{t:<3} SKIPPED (backend error: {e})")
                continue
            candidates.append((f".a{t}", payload, f"single transform #{t}"))
            n_single += 1
            if best_single_size is None or len(payload) < best_single_size:
                best_single_size = len(payload)
            if len(payload) < best_size:
                best_size = len(payload)
            if self.VERBOSE_SINGLE:
                print(f"  [single {t:>3}/256] #{t:<3} -> {len(payload):>10} bytes  "
                      f"(best: {best_size})")
        print(f"  single transforms evaluated : {n_single}")
        ...
        if include_pairs:
            ...
            for idx, (t1, t2) in enumerate(self.pairs):
                if time_limit and time.time() - st > time_limit:
                    print(f"  [pair {idx+1:>5}/{total_pairs}] TIME LIMIT reached, stopping pairs.")
                    break
                try:
                    tr1 = self.fwd_transforms[t1](data)
                except Exception as e:
                    if self.VERBOSE_PAIRS:
                        print(f"  [pair {idx+1:>5}/{total_pairs}] #{idx+1} (#{t1:>3}->#{t2:>3}) SKIPPED (fwd1: {e})")
                    continue
                if not self._verify_lossless(data, tr1, self.rev_transforms[t1]):
                    if self.VERBOSE_PAIRS:
                        print(f"  [pair {idx+1:>5}/{total_pairs}] #{idx+1} (#{t1:>3}->#{t2:>3}) REJECTED (t1 not lossless)")
                    continue
                try:
                    tr2 = self.fwd_transforms[t2](tr1)
                except Exception as e:
                    if self.VERBOSE_PAIRS:
                        print(f"  [pair {idx+1:>5}/{total_pairs}] #{idx+1} (#{t1:>3}->#{t2:>3}) SKIPPED (fwd2: {e})")
                    continue
                back = self.rev_transforms[t1](self.rev_transforms[t2](tr2))
                if back != data:
                    if self.VERBOSE_PAIRS:
                        print(f"  [pair {idx+1:>5}/{total_pairs}] #{idx+1} (#{t1:>3}->#{t2:>3}) REJECTED (pair not lossless)")
                    continue
                try:
                    payload = self._compress_backend(tr2)
                except Exception as e:
                    if self.VERBOSE_PAIRS:
                        print(f"  [pair {idx+1:>5}/{total_pairs}] #{idx+1} (#{t1:>3}->#{t2:>3}) SKIPPED (backend: {e})")
                    continue
                candidates.append((f".p{idx+1}", payload,
                                   f"pair #{idx+1} (#{t1}->#{t2})"))
                n_pair += 1
                if best_pair_size is None or len(payload) < best_pair_size:
                    best_pair_size = len(payload)
                if len(payload) < best_size:
                    best_size = len(payload)
                if self.VERBOSE_PAIRS:
                    print(f"  [pair {idx+1:>5}/{total_pairs}] #{idx+1} "
                          f"(#{t1:>3}->#{t2:>3}) -> {len(payload):>10} bytes  "
                          f"(best: {best_size})")
            print(f"  pair transforms evaluated   : {n_pair} / {total_pairs}")
        ...
        if include_multi:
            rng = random.Random(42)
            tries = 0
            while time_limit and time.time() - st < time_limit and tries < 500:
                tries += 1
                k = rng.randint(2, 3)
                ...
                try:
                    ...
                    for i in seq:
                        a, b = self.pairs[i]
                        cur = self.fwd_transforms[b](self.fwd_transforms[a](cur))
                    back = cur
                    for i in reversed(seq):
                        a, b = self.pairs[i]
                        back = self.rev_transforms[a](self.rev_transforms[b](back))
                    if back != data: continue
                    payload = self._compress_backend(cur)
                    candidates.append((f".m{tries}", payload,
                                       f"multi {k} pairs {seq}"))
                    n_multi += 1
                    if len(payload) < best_size:
                        best_size = len(payload)
                    print(f"  [multi {tries:>4}] {k} pairs {seq} -> "
                          f"{len(payload):>10} bytes  (best: {best_size})")
                except Exception: continue
            print(f"  multi-pair chains evaluated : {n_multi}")

        # ---- .pjp2 ----
        print("  [pjp2] computing raw pipeline ...")
        try:
            ...
            if check_a == data:
                ...
                if len(payload_a) < best_size:
                    best_size = len(payload_a)
                print(f"  [pjp2] -> {len(payload_a):>10} bytes  (best: {best_size})")
        except Exception as e:
            print(f"    Failed: {e}")

        # ---- .pjp3 ----
        print("  [pjp3] computing LZH + hash ...")
        try:
            ...
            if check_b == data:
                ...
                if len(wrapped_b) < best_size:
                    best_size = len(wrapped_b)
                print(f"  [pjp3] -> {len(wrapped_b):>10} bytes  (best: {best_size})")
        except Exception as e:
            print(f"    Failed: {e}")
        ...
        removed = 0
        for p in (infile + ".pjp2", infile + ".pjp3"):
            if os.path.exists(p):
                try: os.remove(p); removed += 1
                except Exception: pass
        try:
            for f in os.listdir(os.path.dirname(infile) or '.'):
                full = os.path.join(os.path.dirname(infile) or '.', f)
                if re.match(rf'^{re.escape(os.path.basename(infile))}\.[apm]\d+$', f):
                    try: os.remove(full); removed += 1
                    except Exception: pass
        except Exception: pass
        if removed: print(f"  Removed {removed} stale output file(s).")
        ...
        if len(ranked) > 1:
            ...
            for ext, payload, label in ranked[1:6]:
                ...
                print(f"  {ext:>10}  {len(payload):>10} bytes  "
    ...
    def decompress_file(self, infile, outfile=""):
        ...
        if m:
            ...
            if not (0 <= idx < len(self.pairs)):
                print(f"Invalid pair index: {idx}"); return False
            t1, t2 = self.pairs[idx]
            with open(infile, 'rb') as f: blob = f.read()
            print(f"Format: pair #{idx+1} (#{t1}->#{t2})")
            r = self._decompress_backend(blob)
            if r is None: print("Backend decode failed"); return False
            try: original = self.rev_transforms[t1](self.rev_transforms[t2](r))
            except Exception as e: print(f"Reverse failed: {e}"); return False
            if not outfile:
                base = os.path.basename(infile)
                outfile = re.sub(r'\.p\d+$', '', base)
        ...
        if m:
            tnum = int(m.group(1))
            if not (1 <= tnum <= 256):
                print(f"Invalid transform: {tnum}"); return False
            with open(infile, 'rb') as f: blob = f.read()
            print(f"Format: .a{tnum} (single transform)")
            r = self._decompress_backend(blob)
            if r is None: print("Backend decode failed"); return False
            try: original = self.rev_transforms[tnum](r)
            except Exception as e: print(f"Reverse failed: {e}"); return False
            if not outfile:
                base = os.path.basename(infile)
        ...
        if blob.startswith(MAGIC):
            ...
            if actual_hash != expected_hash:
                print("*** INTEGRITY FAILURE ***"); return False
    ...
    def full_self_test(self):
        ...
        try:
            c = self._lzh_pipeline(d, time_limit=30)
        ...
        print("\nAlgorithm #58 round-trip...")
        try:
            cases = [b"", b"A", b"AB" * 40,
                     bytes(rng.randint(0, 255) for _ in range(257)),
                     (b"The quick brown fox jumps over the lazy dog. " * 20),
                     b"\x00" * 500, b"\xff" * 500,
                     bytes(rng.randint(0, 255) for _ in range(2000))]
            for d0 in cases:
                tr = self.transform_58(d0)
                rt = self.reverse_transform_58(tr)
                if rt != d0: print(f"  FAIL len={len(d0)}"); return False
            print(f"  PASS #58 on {len(cases)} cases")
        except Exception as e: print(f"  FAIL #58: {e}"); return False
        print("\n[All checks passed]")
        return True
...
def main():
    print(f"{PROGNAME}")
    print("* Exhaustive 65,535-pair tournament -- verbose progress. *")
    print("* Built-in 260,000-word A-Z dictionary + 12 Google Drive files. *\n")
    ...
    while True:
        print("\nMenu:")
        print("1) Compress (exhaustive tournament, all 65,535 pairs)")
        print("2) Decompress (auto-detect .pjp2/.pjp3/.aN/.pN)")
        print("3) Full self-test")
        print("4) Compress (add multi-pair chains)")
        print("5) Toggle verbose: singles={} pairs={}".format(
            c.VERBOSE_SINGLE, c.VERBOSE_PAIRS))
        print("0) Exit")
        ch = input("> ").strip()
        if ch == "1":
            f = input("Input file: ").strip()
            c.compress_file_tournament(f, include_pairs=True, include_multi=False)
        elif ch == "2":
            while True:
                f = input("Compressed file [Enter=cancel]: ").strip()
                if not f: break
                ok = (f.lower().endswith('.pjp2') or f.lower().endswith('.pjp3')
                      or re.search(r'\.[ap]\d+$', f))
                if not ok:
                    print("Only .pjp2/.pjp3/.aN/.pN supported."); continue
            ...
            f = input("Input file: ").strip()
            c.compress_file_tournament(f, include_pairs=True, include_multi=True)
        elif ch == "5":
            c.VERBOSE_SINGLE = not c.VERBOSE_SINGLE
            c.VERBOSE_PAIRS = not c.VERBOSE_PAIRS
            print(f"Verbose singles={c.VERBOSE_SINGLE} pairs={c.VERBOSE_PAIRS}")
        elif ch == "0":
            break
        else:
            print("Invalid.")
